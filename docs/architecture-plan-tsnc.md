...
| `arr` | Arrays | Array cell, growth, appending and removing at the end, slicing, search, joining and the string of an array, sorting with a closure comparator per the `abi` convention or in the order of strings, `String.prototype.split` | Buffer, length and capacity | Growth strategy | `abi`, `gc`, `value`, `str`, `num`, `core:slice` | Passes |
