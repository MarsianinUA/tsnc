...
// Check_Task is always heap-allocated, for the same reason File_Task is: the allocator taken from
// arena below captures it by pointer, so the task must never move.
Check_Task :: struct {
...
// run_checkers makes one task today; from T6.2 one per partition, all of them in the pool at once,
// which is why the answer is a list.
