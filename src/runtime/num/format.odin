...
core:strconv is the engine and the layout is ours, which is the rule of requirements 4.5. What we
borrow is the hard half: the exact decimal expansion of a double and the shortest digits that read
back as the same double. What we cannot borrow is strconv's own formatter, which writes a leading
sign on every number, pads an exponent to two digits where ECMAScript writes 1e-7, and switches to
the exponential form on thresholds that are not 1e21 and 1e-7.
...
to_string :: proc(buf: []byte, value: f64) -> string {
	...
	// The shape of a double for the digit generator: 52 mantissa bits, 11 exponent bits, bias 1023.
	shape := strconv.Float_Info{52, 11, -1023}
	strconv.round_shortest(&d, mantissa, exponent, &shape)
...
// exponent that strconv.round_shortest needs to shorten it. It is the opening of
// strconv.generic_ftoa (core/strconv/generic_float.odin), the part of it that produces digits.
@(private)
